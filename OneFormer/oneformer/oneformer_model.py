...
class OneFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.classification_config_path = Classification_Config_Path
...
####################################################################################################
        ...
        self.cls_model, embed_dim = build_model_from_cfg(cfg, only_teacher=True)

        missing_keys, unexpected_keys = self.cls_model.load_state_dict(torch.load(self.Classification_WEIGHTS, map_location = torch.device('cpu')), strict=False)
        print("Missing keys:", missing_keys)
        print("Unexpected keys:", unexpected_keys)
        self.cls_model.to('cuda')
        self.cls_model.eval()
        self.img_size = cfg.crops.global_crops_size
        self.target_size = cfg.student.num_points
...
####################################################################################################
    ...
    def from_config(cls, cfg):
        ...
        if cfg.MODEL.TEST.TASK == "semantic":
            alpha = cfg.MODEL.alpha_semantic
        elif cfg.MODEL.TEST.TASK == "instance":
            alpha = cfg.MODEL.alpha_instance
        elif cfg.MODEL.TEST.TASK == "panoptic":
            alpha = cfg.MODEL.alpha_panoptic
        ...
        return {
            ...
            "Classification_Config_Path": cfg.MODEL.Classification_Config_Path,
            "Classification_WEIGHTS": cfg.MODEL.Classification_WEIGHTS,
            "alpha": alpha,
        }
...
##################################################################################
                ...
                out_cls_results = out_cls_results.softmax(-1)
                in_cls_results = in_cls_results.softmax(-1)

                cls_results = (in_cls_results**(1 - self.alpha) * out_cls_results**self.alpha).log()
                
                is_void_prob = F.softmax(mask_cls_result, dim=-1)[..., -1:]
...
####################################################################################### 
    def create_class_tensor_no_loop(self, masks, image_path, image):
        ...
        chunks = torch.split(normalized_coords0, self.target_size)

        if chunks[-1].size(0) < self.target_size:
            padding_rows = self.target_size - chunks[-1].size(0)
            padding = torch.rand(padding_rows, 2).to(normalized_coords0)
