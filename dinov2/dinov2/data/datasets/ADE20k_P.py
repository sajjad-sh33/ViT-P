...
class ADE20k_P(Dataset):
    def __init__(self, split, n_points, image_size):
        ...
        # self.root_dir = glob.glob('./ADEChallengeData2016/images/' + split + '/*.jpg
        
        if split=='train':
            with open('./ADEChallengeData2016/train_path.pkl', 'rb') as file:
                self.root_dir = pickle.load(file)
                self.split = "training"
        else:
            with open('./ADEChallengeData2016/valid_path.pkl', 'rb') as file:
                self.root_dir = pickle.load(file)
    ...
    def __getitem__(self, idx):
        ...
        msk_pth =  "./ADEChallengeData2016/annotations/"+ self.split + '/' + splited[0]+'.png'
