...
class COCO_stuff_P(Dataset):
    ...
    def __init__(self, split, n_points, image_size):
        ...
        if split=='train':
            with open('./COCO_stuff_164K/train_coco_stuff.pkl', 'rb') as file:
                self.root_dir = pickle.load(file)     
        else:
            with open('./COCO_stuff_164K/val_coco_stuff.pkl', 'rb') as file:
                self.root_dir = pickle.load(file)
    ...
    def __getitem__(self, idx):
        ...
        img_pth = './COCO_stuff_164K/' + self.split +'2017/'+ self.root_dir[idx] +'.jpg'
        image = Image.open(img_pth)
        # splited = img_pth.split('/')
        # splited = splited[-1].split('.')
         # msk_pth =  "./COCO_stuff_164K/"+ self.split + "2017/"+splited[0]+'.png'
        msk_pth = './COCO_stuff_164K/' + self.split +'2017/'+ self.root_dir[idx] +'.png'
