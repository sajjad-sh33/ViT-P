...
class COCO_stuff_P(Dataset):
    ...
    def __init__(self, split, n_points, image_size):
        ...
        if split=='train':
            with open('./datasets/COCO_stuff_164K/train_coco_stuff.pkl', 'rb') as file:
                self.root_dir = pickle.load(file)     
        else:
            with open('./datasets/COCO_stuff_164K/val_coco_stuff.pkl', 'rb') as file:
                self.root_dir = pickle.load(file)
        ...
        self.id_labels = {}
        w=0
        for COCO_CATEGORIE in COCO_CATEGORIES:
            self.id_labels[COCO_CATEGORIE['id']-1] = w
            w = w+1
        # self.id_labels[255] = 171
    ...
    def __getitem__(self, idx):
        ...
        img_pth = './datasets/COCO_stuff_164K/' + self.split +'2017/'+ self.root_dir[idx] +'.jpg'
        image = Image.open(img_pth)
        # splited = img_pth.split('/')
        # splited = splited[-1].split('.')
         # msk_pth =  "./COCO_stuff_164K/"+ self.split + "2017/"+splited[0]+'.png'
        msk_pth = './datasets/COCO_stuff_164K/' + self.split +'2017/'+ self.root_dir[idx] +'.png'
        # mask_np = cv2.imread(msk_pth, cv2.IMREAD_GRAYSCALE)
        mask_np = Image.open(msk_pth)
        if self.augmentation:
            image,mask_np = self.augmentation(image, mask_np)
